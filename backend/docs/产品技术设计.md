...
| 目标规模 | 1万+并发用户在线 |
| 技术栈 | Go 1.22+ / Gin / WebSocket(gorilla) / MySQL 8.0 / Redis 7 / RabbitMQ / 本地文件存储 / JWT |
| 部署模式 | 单机部署 |
...
```
  ...
  /ws?token=JWT         │    ├─ ConnectionManager (sync.Map)               │
                        ...
                        │    │   ├─ moment_push → outbox + big-user check + fanout to timelines             │
                        │    │   ├─ like_persist → MySQL                    │
...
| 朋友圈 | `timeline:{userID}` | ZSet | Feed流收件箱（推来的动态ID） | 3天 + timelineMaxLen上限 |
| 朋友圈 | `moment_outbox:{userID}` | ZSet | Feed流寄件箱（自己发布的动态，拉取源） | timelineMaxLen上限 |
| 朋友圈 | `moment:big_users` | Set | 大V用户集合（好友数>阈值） | 永不过期(sticky) |
| 朋友圈 | `moment:likes:{momentID}` | Set | 点赞用户集合 | likeCacheTTL (默认7天) |
| 朋友圈 | `moment:like_count:{momentID}` | String | 点赞计数 | likeCacheTTL |
| 朋友圈 | `moment:like_loaded:{momentID}` | String | 点赞缓存预热完成标记 | likeCacheTTL |
| 朋友圈 | `moment:like_lock:{momentID}` | String(SETNX) | 点赞预热互斥锁 | 5s |
...
消息JSON格式:
{
  ...
  "receiverID": 200,
  "content":    "hello",
  "msgType":    1,                 // 1=文字,2=图片,3=视频,5=系统通知,6=撤回通知
...
### 8.1 推拉结合模式（Push-Pull Hybrid）

朋友圈 Feed 流采用推拉结合策略，核心思路：**普通用户走推（写扩散），大V用户走拉（读时合并）**。这比纯推或纯拉都更优——纯推对大V内存不可接受，纯拉对普通用户读延迟不可接受。
...
用户A发布动态 ──POST /api/v1/moment/publish──→ GoIM Server
                                                  ...
                                                  ├─ MQ moment_push
                                                  │
                                             MQ Consumer (MomentFeedConsumer)
                                                  │
                                                  ├─ 1. 写入作者寄件箱 moment_outbox:{A}（所有动态必写）
                                                  │
                                                  ├─ 2. 私密动态(visibility=3) → 停止扩散
                                                  │
                                                  ├─ 3. 按好友数分流：
                                                  │     好友数 > bigUserFriendThreshold(默认500)
                                                  │     ├─ 标记为大V: SADD moment:big_users {A}
                                                  │     └─ 跳过写扩散（好友读取时从寄件箱拉取）
                                                  │
                                                  │     好友数 ≤ 阈值（普通用户）
                                                  │     └─ 写扩散: pipeline ZADD timeline:{好友ID} (momentID, ts)
                                                  │        + ZREMRANGEBYRANK 裁剪到 timelineMaxLen 条
```
...
```
纯推（100%写扩散）：
  大V（5000好友）发布一条 → 写5000个timeline ZSet → 5000次ZADD
  大V日均10条 → 50000次ZADD/天 → Redis压力巨大 → 内存占用爆炸

纯拉（100%读扩散）：
  普通用户（50好友）刷Feed → 读50个好友的outbox → 50次ZREVRANGEBYSCORE
  每次刷Feed都要遍历好友 → 读延迟高 → 用户体验差

推拉结合：
  普通用户 → 推（读自己的timeline，一次ZREVRANGEBYSCORE，极快）
  大V → 拉（新粉读大V寄件箱，大V发布无扩散开销）
  关键洞察：好友数分布极度不均（少数大V占多数好友关系），
  对大V用拉避免扩散，对普通用户用推保证读速度——对症下药。
```

### 8.2 双ZSet架构：收件箱 + 寄件箱

```
timeline:{userID} → ZSet（收件箱 — 普通好友推送来的动态）
  score = timestamp (毫秒)
  value = momentID (int64, 只存ID，详情从MySQL批量查)
  保护: ZREMRANGEBYSCORE 3天过期 + ZREMRANGEBYRANK 保留最新 timelineMaxLen 条

moment_outbox:{userID} → ZSet（寄件箱 — 用户自己发布的动态，拉取源）
  score = timestamp (毫秒)
  value = momentID
  保护: ZREMRANGEBYRANK 保留最新 timelineMaxLen 条
  用途:
    a) 作者自见（自己的动态不出现在收件箱，从寄件箱拉取）
    b) 大V动态源（好友读取时从此拉取）
    c) 补充容错（即使写扩散漏了某个好友，仍可从寄件箱拉取）

moment:big_users → Set（大V用户集合，sticky标记，只增不删）
  用途: SMISMEMBER 批量判定好友中哪些是大V，决定是否需要拉取寄件箱
```

**为什么只存 momentID 而非完整 JSON？**

- ZSet 只存 8 字节 momentID，而非 ~500 字节完整动态 JSON → 内存节省 50倍+
- 详情从 MySQL 批量查（SELECT ... WHERE id IN (...)，走主键索引，极快）
- 动态内容可能被编辑（虽然当前不支持），存 ID 而不是快照保证了数据一致性

### 8.3 Feed 拉取流程（多源归并 + 复合游标分页）

```
用户刷Feed ──GET /api/v1/moment/feed?cursor={cursor}&limit=20──→ GoIM Server
                                                                │
                                                                ├─ 1. 确定Feed源:
                                                                │    源1: moment_outbox:{userID}        (自己的寄件箱, pull=true)
                                                                │    源2: timeline:{userID}              (自己的收件箱, pull=false)
                                                                │    遍历好友列表 → FilterBigUsers(friendIDs)
                                                                │    源3+: moment_outbox:{大V好友ID}     (大V好友寄件箱, pull=true)
                                                                │
                                                                ├─ 2. 各源取一页 (limit+1用于判断是否有下一页)
                                                                │    GetOutboxPage / GetTimelinePage:
                                                                │      ZREVRANGEBYSCORE key maxTs (开区间) LIMIT 0 limit+1
                                                                │      复合游标 (maxTs, maxID):
                                                                │        返回严格早于游标的条目: ts<maxTs OR (ts==maxTs AND id<maxID)
                                                                │        同毫秒内继续翻页，杜绝重复/漏读
                                                                │
                                                                ├─ 3. Go内存归并:
                                                                │    按momentID去重(多源可能重叠)
                                                                │    按(ts desc, id desc)排序
                                                                │    取前limit条，第limit+1条 -> 生成nextCursor
                                                                │
                                                                ├─ 4. MySQL批量补全详情 (SELECT ... WHERE id IN (...))
                                                                │    读时可见性过滤: 私密动态(visibility=3)仅作者本人可见
                                                                │
                                                                ├─ 5. Redis Pipeline批量读点赞状态
                                                                │    ensureLikesLoaded (warm-up) → GetMomentLikeStats
                                                                │    ├─ GET moment:like_count:{momentID}  → likeCount
                                                                │    └─ SISMEMBER moment:likes:{momentID} {viewerID} → LikedByMe
                                                                │
                                                                ├─ 组装响应返回 {moments, nextCursor}
```

**复合游标分页的核心价值：**

传统 offset 分页在 Feed 场景有严重问题：翻页过程中新动态插入会导致偏移（读到重复）、删除导致漏读。复合游标 `(ts, id)` 天然避免了这些问题：

- 游标编码为 `base64("{ts}_{id}")`，解码后作为各源的 ZREVRANGEBYSCORE 上界
- `ts` 相同毫秒内的多条动态通过 `id` 继续分页（ZREVRANGEBYSCORE max 用 `(maxTs` 开区间）
- 与深分页 offset 衰减退化为 O(S+N) 不同，ZREVRANGEBYSCORE 始终 O(log N + M)
...
func decodeCursor(cursor string) (int64, int64, error) {
    if cursor == "" { return -1, 0, nil } // 首页: maxTs<0 由 Redis 层识别
    // ... base64解码 + 拆分 "_"
}
```

### 8.4 点赞高并发 — Lua原子 + 按需预热 + 批量削峰
...
用户点赞 ──WebSocket──→ GoIM Server
                          ...
                          ├─ 1. 验证动态存在 (MySQL)
                          │
                          ├─ 2. 按需预热 (ensureLikesLoaded):
                          │     ├─ EXISTS moment:like_loaded:{momentID} → 已预热，跳过
                          │     └─ 未预热 → SETNX moment:like_lock:{momentID} 抢锁(5s TTL)
                          │         ├─ 抢到锁 → MySQL GetMomentLikers → SADD 载入 Set + SET count
                          │         └─ 未抢到 → 轮询 loaded 标记 ~200ms → 尽力而为返回
                          │
                          ├─ 3. Lua原子点赞 (moment:likes:{momentID} + moment:like_count:{momentID}):
                          │     SADD set userID
                          │     ├─ 返回1(新增) → INCR count → return {changed=1, count}
                          │     └─ 返回0(已存在) → GET count → return {changed=0, count} (幂等)
                          │
                          ├─ 4. changed=true → 发布 like_persist MQ事件
...
取消点赞 ──WebSocket──→ GoIM Server
                          ...
                          ├─ Lua原子取消赞:
                          │     SREM set userID
                          │     ├─ 返回1(确实移除) → DECR count(夹到≥0) → return {changed=1, count}
                          │     └─ 返回0(本就不在) → GET count → return {changed=0, count} (幂等)
                          │
                          ├─ changed=true → 发布 like_persist MQ事件(action=unlike)
```

**点赞 Lua 脚本（原子判重 + 计数）：**

```lua
-- luaMomentLike: KEYS[1]=点赞集合, KEYS[2]=计数key; ARGV[1]=userID
local added = redis.call('SADD', KEYS[1], ARGV[1])
if added == 1 then
    local c = redis.call('INCR', KEYS[2])
    return {1, c}   -- changed=1, 新赞
end
return {0, tonumber(redis.call('GET', KEYS[2]) or '0')}  -- changed=0, 幂等
```

**按需预热机制（防缓存击穿）：**

点赞缓存不是系统启动时全量加载，而是首次访问某条动态的点赞数据时按需从 MySQL 回源。预热通过 `moment:like_loaded:{momentID}` 标记避免反复回源，通过 `moment:like_lock:{momentID}` NX 锁避免并发回源（缓存击穿）。TTL 默认为 7 天（可配置），冷数据到期自动淘汰。

**点赞落库 — 攒批削峰（LikePersistConsumer）：**

不逐条 INSERT，而是攒批处理：

```
攒批循环:
  ├─ 攒够 batchSize(默认200)条 → flush
  └─ 每 flushInterval(默认500ms) → flush（防止低流量时无限等待）

flush 逻辑:
  1. 按 (momentID, userID) 折叠: map[key] = last-action(按到达顺序覆盖)
  2. like动作 → 批量 INSERT IGNORE (ODKU语义，幂等)
  3. unlike动作 → 批量 DELETE WHERE (moment_id, user_id) IN (...)
  4. 全部成功 → 整批 ack
  5. 任一失败 → 整批 nack + requeue（幂等保证重试安全）

一致性保证:
  - INSERT IGNORE / DELETE 均幂等
  - last-action-wins: 同一Key的多次事件折叠为最终态
  - 单 goroutine 顺序 flush → 跨批时间先后有保证
  - Redis 已生效的参数优先（写路径 Redis 先行，MQ 异步补 MySQL）
```
...
| Key | 类型 | TTL | 用途 |
|-----|------|-----|------|
| `moment:likes:{momentID}` | Set | likeCacheTTL (默认7天) | 点赞用户ID集合（SADD/SREM/SISMEMBER） |
| `moment:like_count:{momentID}` | String | likeCacheTTL | 点赞计数（GET/INCR/DECR，与Set同步维护） |
| `moment:like_loaded:{momentID}` | String | likeCacheTTL | 预热完成标记（EXISTS判是否已回源） |
| `moment:like_lock:{momentID}` | String(SETNX) | 5s | 预热互斥锁（防缓存击穿） |

**面试论述要点：为什么用四个独立 Key 而非一个 Hash？**

- 分离读取路径：count 是 String(GET→Int64)，likes 是 Set(SISMEMBER→Bool)，各自用最合适的 Redis 命令
- 原子操作：点赞 Lua 脚本仅需操作 Set + String，若用 Hash 则需要 HINCRBY + SADD 跨类型操作，无法在一个 Lua 脚本中表达
- loaded 标记独立 TTL 语义：标记过期=缓存失效=下次重新预热，与数据本身生命周期解耦
- lock 用独立短 TTL(5s)：即使抢锁方崩溃，锁很快自动释放，不阻塞后续预热
...
评论 ──POST /api/v1/moment/comment──→ GoIM Server
                                      │
                                      ├─ 验证动态存在
                                      ├─ 生成评论ID (time.Now().UnixNano())
                                      ├─ MySQL写moment_comments表
                                      ├─ HTTP响应返回commentID
```

评论走简化路径：直接写 MySQL，不经过 Redis 缓存和 MQ。对于简历项目，评论不是高并发瓶颈，简化设计更清晰。
...
动态发布时指定 visibility:
  1 = 所有好友可见（默认）→ 推拉结合扩散
  2 = 指定好友可见（friendIDs列表）→ 只写扩散到指定好友收件箱
  3 = 仅自己可见 → 仅写作者寄件箱，不扩散；Feed读取时非作者过滤

读时过滤: GetFeed中，visibility=3 AND authorID != viewerID → 跳过
```
...
CREATE TABLE private_messages (
    ...
    msg_type      TINYINT NOT NULL DEFAULT 1,     -- 1文字/2图片/3视频/5系统/6撤回
    created_at    DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
| **朋友圈** | | | | |
| | `timeline:{userID}` | ZSet | 3天+timelineMaxLen上限 | Feed流收件箱（普通好友推来的动态） |
| | `moment_outbox:{userID}` | ZSet | timelineMaxLen上限 | Feed流寄件箱（自己发布的动态，拉取源） |
| | `moment:big_users` | Set | 永不过期(sticky) | 大V用户集合 |
| | `moment:likes:{momentID}` | Set | likeCacheTTL(默认7天) | 点赞用户集合 |
| | `moment:like_count:{momentID}` | String | likeCacheTTL | 点赞计数 |
| | `moment:like_loaded:{momentID}` | String | likeCacheTTL | 点赞预热完成标记 |
| | `moment:like_lock:{momentID}` | String(SETNX) | 5s | 点赞预热互斥锁 |
| **其他** | | | | |
...
| `moment_push` | MomentService | MomentFeedConsumer | 动态发布数据 | 推拉结合: 写寄件箱 + 大V判定(SADD big_users) + 普通用户pipeline写扩散到好友收件箱 |
| `like_persist` | MomentService | LikePersistConsumer | 点赞事件(like/unlike) | 攒批削峰: last-action-wins折叠 + 批量INSERT IGNORE / DELETE |
| `comment_persist` | MomentService | CommentPersistConsumer | 评论数据 | 写MySQL（当前评论直接写MySQL，MQ为扩展预留） |
...
1. Lua原子操作（消除竞态）：
   SADD判重 + INCR计数在单个Lua脚本中原子执行
   0次网络往返（脚本内部直接调用Redis命令）
   无竞态窗口：要么全部通过，要么全部拒绝
   天然幂等：重复点赞返回 changed=false，计数不变
   1000点赞 → 100ms → 完美

2. 按需预热（防缓存击穿）：
   首次访问某动态的点赞数据时，从MySQL回源加载到Redis
   loaded标记避免反复回源，NX锁避免并发回源（缓存击穿）
   TTL 7天自动淘汰冷数据
   好处：不需要系统启动时全量预热，按需加载，内存只存热数据

3. 批量削峰落库（保护MySQL）：
   不逐条INSERT，而是攒批处理（batchSize=200 / flushInterval=500ms）
   批内按 (momentID,userID) 折叠为最终动作（last-action-wins）
   分两条SQL：批量 INSERT IGNORE（点赞） + 批量 DELETE（取消赞）
   整批成功→统一ack，失败→整批nack+requeue（幂等保证重试安全）
   折叠加批量化：500条事件可能折叠为50个实际变化 → MySQL写入量减少90%+

对比旧方案（SISMEMBER+SADD+HINCRBY逐命令 + 逐条ODKU落库）：
  旧：多次网络往返 + 逐条MySQL写入 → 高并发时MySQL连接池枯竭
  新：单次Lua原子 + 批量削峰 → MySQL写入量降低一个数量级
```
...
> 6. 点赞所有人可见 — 不做共同好友过滤，简化设计并保持技术论述简洁
