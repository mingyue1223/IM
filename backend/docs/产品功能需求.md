...
### 1.2 项目定位
面向1万+并发用户的开源IM后端系统，采用Go语言单体架构，核心解决消息可靠性（不丢不重）问题，在此基础上扩展群聊、朋友圈功能。作为简历展示项目，技术深度是核心卖点。
