...
| `syncReq` | `{ lastSyncTime, batchSize }` | 拉取离线私聊及群聊消息；首次同步使用 `lastSyncTime: 0`。`batchSize` 省略或小于等于 0 时为 50。|
...
| `syncBatch` | `{ msgs, hasMore, syncTime }` | 合并消息并以 `syncTime` 作为下一批 `lastSyncTime`；`hasMore=true` 时继续请求。|
...
5. 重连时先完成 `syncBatch` 分页，再处理 `convSync`；按 `msgId`/`serverMsgId` 去重。
