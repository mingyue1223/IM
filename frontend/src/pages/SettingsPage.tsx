...
export function SettingsPage() {
  ...
  const updatePreferences = (nextNotifications: boolean, nextPreview: boolean) => {
  ...
  const uploadAvatar = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try { const response = await uploadApi.avatar(file); setAvatarUrl(response.url.startsWith("http") ? response.url : `${env.staticBaseUrl}${response.url}`); }
  ...
  return (
    <>
      ...
      <section className="module-main settings-main">
        ...
        {section === "notifications" && <SettingsSection description="选择何时以及如何接收新消息提醒。" title="通知">{previewMode && <div className="settings-preview-note">预览模式下设置仅在当前页面生效，不会写入服务器。</div>}{settingsQuery.isError && <div className="settings-sync-error">暂时无法读取服务端设置，请检查后端连接后重试。</div>}<div className="settings-group"><Switch checked={notifications} description="收到新消息时显示桌面通知" disabled={settingsQuery.isLoading || settingsMutation.isPending} label="消息通知" onCheckedChange={(checked) => previewMode ? setNotifications(checked) : updatePreferences(checked, preview)} /><Switch checked={preview} description="在系统通知中显示消息内容" disabled={settingsQuery.isLoading || settingsMutation.isPending} label="消息预览" onCheckedChange={(checked) => previewMode ? setPreview(checked) : updatePreferences(notifications, checked)} /></div>{settingsMutation.isError && <p className="settings-save-error">保存失败，当前显示可能尚未同步到服务器。</p>}</SettingsSection>}
