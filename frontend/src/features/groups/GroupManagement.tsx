...
export function GroupManagementDrawer({ conversation, open, onClose }: GroupManagementDrawerProps) {
  ...
  return <><Drawer description={`${members.length} 位成员 · 最多 ${group?.max_members ?? 500} 人`} onClose={onClose} open={open} title="群聊资料"><div className="group-profile-head"><Avatar name={group?.name ?? conversation.name} size="xl" /><div><h3>{group?.name ?? conversation.name}</h3><p>{group?.notice || "暂无群公告"}</p></div>{canManage && <Button onClick={() => setEditing((value) => !value)} size="sm" variant="secondary">{editing ? "取消编辑" : "编辑资料"}</Button>}</div>
    ...
    <section className="group-members"><header><h3><Users size={16} />群成员</h3><span>{members.length}</span></header>{sortedMembers.map((member) => { const isMuted = Boolean(member.muted_until && new Date(member.muted_until).getTime() > Date.now()); const canToggleMemberMute = member.user_id !== currentUserId && member.role !== 2 && (isOwner || (canManage && member.role === 0)); return <div className="group-member-row" key={member.user_id}><Avatar name={member.username || `用户 ${member.user_id}`} size="sm" src={member.avatar_url} /><div className="group-member-copy"><strong>{member.user_id === currentUserId ? `${member.username || "我"}（我）` : member.username || `用户 #${member.user_id}`}</strong><small>用户 #{member.user_id} · {roleNames[member.role]}{isMuted ? " · 已禁言" : ""}</small></div><span className={`role-badge role-badge--${member.role}`}>{member.role === 2 ? <Crown size={11} /> : member.role === 1 ? <ShieldCheck size={11} /> : null}{roleNames[member.role]}</span><div className="group-member-actions">{isOwner && member.role !== 2 && <IconButton label={member.role === 1 ? "取消管理员" : "设为管理员"} onClick={() => updateRole(member)}><ShieldCheck size={15} /></IconButton>}{isOwner && member.role !== 2 && <IconButton label="转让群主" onClick={() => setDanger({ type: "transfer", memberId: member.user_id })}><Crown size={15} /></IconButton>}{canToggleMemberMute && <IconButton label={isMuted ? "解除禁言" : "禁言 10 分钟"} onClick={() => toggleMemberMute(member)}>{isMuted ? <Volume2 size={15} /> : <VolumeX size={15} />}</IconButton>}{canManage && member.role === 0 && member.user_id !== currentUserId && <IconButton label="移除成员" onClick={() => setDanger({ type: "remove", memberId: member.user_id })}><X size={15} /></IconButton>}</div></div>; })}</section>
