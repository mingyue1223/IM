...
export interface MomentView extends Moment { author: string; time: string; comments: MomentCommentView[]; }
...
export function useMoments() {
  ...
  const [localComments, setLocalComments] = useState<Record<number, MomentCommentView[]>>({});
  ...
  const publishMutation = useMutation({ mutationFn: (input: { content: string; visibility: 1 | 2 | 3 }) => momentsApi.publish(input), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["moments", "feed"] }) });
  ...
  const posts: MomentView[] = useMemo(() => {
    ...
    return (feedQuery.data?.pages.flatMap((page) => page.moments) ?? []).map((post) => ({ ...post, author: post.author_id === currentUser?.id ? currentUser.username : `用户 #${post.author_id}`, time: relativeTime(post.created_at), comments: localComments[post.id] ?? [] }));
  }, [currentUser, feedQuery.data, localComments, previewMode, previewPosts]);

  const publish = async (content: string, visibility: 1 | 2 | 3) => {
    if (previewMode) {
      setPreviewPosts((current) => [{ id: Date.now(), author_id: currentUser?.id ?? 10086, author: currentUser?.username ?? "我", content, visibility, created_at: new Date().toISOString(), like_count: 0, liked_by_me: false, time: "刚刚", comments: [] }, ...current]);
      return;
    }
  ...
  const addComment = async (postId: number, content: string) => {
    ...
    else setLocalComments((current) => ({ ...current, [postId]: [...(current[postId] ?? []), comment] }));
  };
  const deleteComment = async (postId: number, commentId: number) => {
    ...
    else setLocalComments((current) => ({ ...current, [postId]: (current[postId] ?? []).filter((comment) => comment.id !== commentId) }));
  };

  return { posts, publish, toggleLike, addComment, deleteComment, isLoading: !previewMode && feedQuery.isLoading, isError: !previewMode && feedQuery.isError, hasNextPage: feedQuery.hasNextPage, fetchNextPage: feedQuery.fetchNextPage, isFetchingNextPage: feedQuery.isFetchingNextPage, isMutating: publishMutation.isPending || likeMutation.isPending || commentMutation.isPending || deleteCommentMutation.isPending };
}
